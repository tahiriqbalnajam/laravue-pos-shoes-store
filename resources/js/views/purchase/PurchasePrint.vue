  ...
  <div class="app-container">
    <div id="printMe">
      ...
      <table class="tblwdborder idlprint-table">
        ...
        <tr v-for="product in sale.products" v-bind="product.product.id">
          ...
          <td align="right" valign="center">{{ getTotalwdDisc(product.price,product.quantity, product.discount1, product.discount2) }}</td>
        </tr>
      </table>
      <table style="width:100%">
        <tr>
...
const salePro = new Resource('purchase');
export default {
  name: 'Printinvoice',
  saleid: 45,
  components: { },
  ...
  methods: {
    getTotalwdDisc(price, qty, disc1, disc2) {
      let total = price * qty;
      if (disc1){
        total = total - (total * (disc1 / 100));
        console.log(total);
      }
      if (disc2) {
        total = total - (total * (disc2 / 100));
      }
