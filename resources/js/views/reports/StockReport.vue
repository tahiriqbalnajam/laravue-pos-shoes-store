    ...
    <div id="printMe">
      ...
      <table class="tblwdborder idlprint-table">
        ...
        <tr v-for="product in products" v-bind="product.id">
          <td width="50%">{{ product.code }}||{{ product.name }} </td>
          <td align="right" valign="center">{{ product.qty }}</td>
          <td align="right" valign="center">{{ product.sale }}</td>
        </tr>
        <tr>
          <td colspan="2" />
...
const salePro = new Resource('get_stock_print');
export default {
  name: 'Printinvoice',
  saleid: 45,
  components: { },
  ...
  methods: {
    ...
    async getProduct() {
      const { data } = await salePro.list();
      this.products = data.sales;
      this.qty = this.products.map(element => parseInt(element.sale)).reduce((a, b) => a + b, 0);
    },
    print() {
